...
import { createCompany, generateLicence, getCompanyDetails, updateCompany } from '../../../actions/apiCall';
...
export default function UpdateClient() {
    ...
    const getCompanyDetailsFun = () => {
        ...
        getCompanyDetails(data).then(result => {
            if(result.data.success){
                let comp = result.data.response.company;
                let user = result.data.response.user[0];
                let license = result.data.response.license;
                setCompName(comp.companyname);
                setCompEmail(comp.email);
                setCompPhnNo(comp.phone);   
                setCompAddress(comp.address);
                setCompNoOfEmps(comp.noemp);
                setCompTaxId(comp.taxid);

                setUserName(user.name);
                setUserEmail(user.email);
                setUserPhnNo(user.phone);
                setUserAddress(user.address);

                setLicenseKey(license.licensekey);
                setLicenseStartTime(moment(license.start_time).format("YYYY-MM-DD"));
                setLicenseEndTime(moment(license.end_time).format("YYYY-MM-DD"));
            } else{
    ...
    const updateCompanyFun = () => {
        ...
        if (err === 0) {
            ...
            updateCompany(data).then(result => {
                if (result.data.success) {
    ...
    const generteLicenseFun = () => {
        generateLicence().then(result => {
            if (result.data.success) {
                // successMsg("HIIIIIIIIIIIIIIIIIIII");
    ...
    return (
        <Page
            ...
            updateCompanyFun={updateCompanyFun}
        />
    )
}
